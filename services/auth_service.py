...
def login_face(image_bytes: bytes, face_bbox: tuple[int, int, int, int]) -> dict[str, Any]:
    if USE_MOCK:
        if not _MOCK_USERS:
            return success({"user_id": "demo_customer", "display_name": "Demo Customer", "role": "customer"})
        return success(next(iter(_MOCK_USERS.values())))

    files = {"image": ("face.jpg", image_bytes, "image/jpeg")}
    data = {"face_bbox": json.dumps(face_bbox)}
